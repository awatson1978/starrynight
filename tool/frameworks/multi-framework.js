...
module.exports = function(npmPrefix, callback){
  ...
  var tinyTestArgs = ['test-packages', '--once', '--driver-package', 'test-in-console', '-p', 10015];
  ...
  var meteor = spawn((process.env.TEST_COMMAND || 'meteor'), nightwatchArgs, {cwd: workingDir});
  meteor.stdout.pipe(process.stdout);
  meteor.stderr.pipe(process.stderr);
  ...
  meteor.stdout.on('data', function startTesting(data) {
    ...
    request("http://localhost:3000", function (error, httpResponse) {
       ...
       if(error){
         console.log("No app is running on http://localhost:10015.  Try launching an app with 'meteor run'.");
         console.error(error);
